...
```bash
helm create XXXXX-chart
```

- See the files of XXXXX-chart.
...
```bash
rm -rf XXXXX-chart/templates/*
```

- Create a `configmap.yaml` file under `XXXXX-chart/templates` folder with following content.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: XXXXX-chart-config
data:
  myvalue: "Hello World"
```

- Install the XXXXX-chart.

```bash
helm install helm-demo XXXXX-chart
```
...
```bash
kubectl get cm
kubectl describe cm XXXXX-chart-config
```
...
- Let's create our own values and use it within the template. Update the `XXXXX-chart/values.yaml` as below.

```yaml
course: DevOps
```

- Edit the XXXXX-chart/templates/configmap.yaml as below.
...
metadata:
  name: XXXXX-chart-config
data:
  myvalue: "XXXXX-chart configmap example"
...
```bash
helm install --debug --dry-run mydryrun XXXXX-chart
```

- Install the XXXXX-chart.

```bash
helm install myvalue XXXXX-chart
```
...
```bash
helm install --debug --dry-run setflag XXXXX-chart --set course=AWS
```
...
```bash
helm install --debug --dry-run builtin-object XXXXX-chart
```

- Let's try more examples to get more clarity. Update the `XXXXX-chart/values.yaml` as below.
...
- Update the `XXXXX-chart/templates/configmap.yaml` as below.
...
```bash
helm install --debug --dry-run morevalues XXXXX-chart
```
...
- To add installation notes to your chart, simply create a `XXXXX-chart/NOTES.txt` file. This file is plain text, but it is processed like a template and has all the normal template functions and objects available.

- Let's create a simple `NOTES.txt` file under `XXXXX-chart/templates` folder.
...
```bash
helm install notes-demo XXXXX-chart
```
...
```bash
mkdir my-charts
mv XXXXX-chart-0.1.0.tgz my-charts
...
entries:
  ...
  - apiVersion: v2
    ...
    type: application
    urls:
    - http://<public ip>:8080/XXXXX-chart-0.1.0.tgz
    version: 0.1.0
...
```bash
cd my-charts
curl --data-binary "@XXXXX-chart-0.1.0.tgz" http://<public ip>:8080/api/charts
```
...
- Let's see how to maintain the chart version. In `XXXXX-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm package XXXXX-chart
mv XXXXX-chart-0.1.1.tgz my-charts
helm repo index my-charts --url http://<public-ip>:8080
```
...
```bash
cd my-charts
curl --data-binary "@XXXXX-chart-0.1.1.tgz" http://<public ip>:8080/api/charts
```
...
- In XXXXX-chart/Chart.yaml, set the `version` value to `0.2.0`and push the chart.

```bash
cd
helm cm-push XXXXX-chart mylocalrepo
```
...
```bash
helm cm-push XXXXX-chart mylocalrepo --version="1.2.3"
```
...
```bash
helm cm-push XXXXX-chart mylocalrepo --version="1.2.4"
```
...
helm repo update
helm search repo mylocalrepo
helm install from-local-repo mylocalrepo/XXXXX-chart
```
...
```bash
helm cm-push XXXXX-chart mylocalrepo --version="1.2.5"
helm repo update
```
...
```bash
helm upgrade from-local-repo mylocalrepo/XXXXX-chart
```
...
```bash
helm upgrade from-local-repo mylocalrepo/XXXXX-chart --version 1.2.4
```

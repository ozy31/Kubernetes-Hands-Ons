...
- Create yaml file named `XXXXX-deploy.yaml` and explain its fields.
...
- Update the XXXXX-deploy.yaml as below.
...
- Now, we will update XXXXX-deploy.yaml as below. We just add `nodeSelector` field.
...
- We will update XXXXX-deploy.yaml as below. We will add an affinity field instead of nodeSelector.
...
- This time we will test `preferredDuringSchedulingIgnoredDuringExecution`. Update XXXXX-deploy.yaml as below. This time, we change `requiredDuringSchedulingIgnoredDuringExecution` with  `preferredDuringSchedulingIgnoredDuringExecution`.
...
- create yaml file and named `XXXXX-db.yaml`.
...
-  Create yaml file named `XXXXXshop-deploy.yaml`.
...
```bash
kubectl apply -f XXXXXshop-deploy.yaml
```

- List the pods and notice that the XXXXXshop pods are assigned to the same nod with XXXXX-db pod.
...
```bash
kubectl delete -f XXXXXshop-deploy.yaml
kubectl delete -f XXXXX-db.yaml
```
...
```bash
kubectl taint nodes kube-worker-1 XXXXX=way:NoSchedule
```

- This command places a taint on node kube-worker-1. The taint has key XXXXX, value way, and taint effect NoSchedule. This means that no pod will be able to schedule onto kube-worker-1 unless it has matching toleration.

...
- Create a `name-deploy.yaml` and input text below. Pay attention that image version is 1.0.
...
metadata:
  ...
  labels:
    app: container-info
  annotations:
    kubernetes.io/change-cause: deploy/name-deploy is set as container-info=nameway/container-info:1.0
spec:
    replicas: 2
  selector:
    matchLabels:
  ...
  template:
    ...
    spec:
      containers:
      - name: container-info
        image: nameway/container-info:1.0
        ports:
        - containerPort: 80
```
...
- List the `Deployment`, `ReplicaSet` and `Pods` of `name-deploy` deployment using a label and note the name of ReplicaSet.

```bash
kubectl get deploy,rs,po -l app=container-info
```

- Describe deployment and note the image of the deployment. In our case, it is nameway/container-info:1.0.

```bash
kubectl describe deploy name-deploy
```
...
```bash
kubectl rollout history deploy name-deploy --revision=1
```

- Upgrade image.

```bash
kubectl set image deploy name-deploy container-info=nameway/container-info:2.0
```
...
```bash
kubectl rollout history deploy name-deploy --revision=1
kubectl rollout history deploy name-deploy --revision=2
```

- List the `Deployment`, `ReplicaSet` and `Pods` of `name-deploy` deployment using a label and explain ReplicaSets.
...
...
...
    kubernetes.io/change-cause: kubectl set image deploy name-deploy container-info=nameway/container-info:3.0
...
...
    spec:
      containers:
      - image: nameway/container-info:3.0
...
...
```
...
kubectl rollout history deploy name-deploy --revision=1
kubectl rollout history deploy name-deploy --revision=2
kubectl rollout history deploy name-deploy --revision=3
```
...
```bash
kubectl rollout undo deploy name-deploy --to-revision=1
```
...
kubectl rollout history deploy name-deploy --revision=2
kubectl rollout history deploy name-deploy --revision=2
kubectl rollout history deploy name-deploy --revision=4
```
...
kubectl create deployment name-ns --image=nginx -n=name-namespace
```
...
- List the deployments in `name-namespace`.

```bash
kubectl get deployment -n name-namespace
```
...
```bash
kubectl delete namespaces name-namespace
```
...
- Create a `XXXXX-pv.yaml` file using the following content with the volume type of `hostPath` to build a `PersistentVolume` and explain fields.
...
- Create the PersistentVolume `XXXXX-pv-vol`.

```bash
kubectl apply -f XXXXX-pv.yaml
```
...
- Create a `XXXXX-pv-claim.yaml` file using the following content to create a `PersistentVolumeClaim` and explain fields.
...
- Create the PersistentVolumeClaim `XXXXX-pv-claim`.

```bash
kubectl apply -f XXXXX-pv-claim.yaml
```
...
- View information about the `PersistentVolumeClaim` and show that the `PersistentVolumeClaim` is bound to your PersistentVolume `XXXXX-pv-vol`.

```bash
kubectl get pvc XXXXX-pv-claim
```
...
- Create a `XXXXX-pod.yaml` file that uses your PersistentVolumeClaim as a volume using the following content.
...
spec:
  volumes:
    - name: XXXXX-pv-storage
      persistentVolumeClaim:
        claimName: XXXXX-pv-claim
  containers:
    - name: XXXXX-pv-container
      image: nginx
      ports:
        - containerPort: 80
          name: "http-server"
...
```bash
kubectl get pod XXXXX-pod
```

- Open a shell to the container running in your Pod.

```bash
kubectl exec -it XXXXX-pod -- /bin/bash
```
...
- Expose the XXXXX-pod pod as a new Kubernetes service on master.

```bash
kubectl expose pod XXXXX-pod --port=80 --type=NodePort
```
...
- Check the browser (`http://<public-workerNode-ip>:<node-port>`) that XXXXX-pod is running.

- Delete the `Pod`, the `PersistentVolumeClaim` and the `PersistentVolume`.

```bash
kubectl delete pod XXXXX-pod
kubectl delete pvc XXXXX-pv-claim
kubectl delete pv XXXXX-pv-vol
```
